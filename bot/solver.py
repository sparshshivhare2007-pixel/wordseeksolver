...
class WordleSolver:
    ...
    async def _start_new_game(self):
        ...
        with open(Config.WORDLIST_FILE, "r", encoding="utf-8") as f:
            self.possible = [
                w.lower() for w in json.load(f) if len(w) == 5
            ]

        await self._send_first_guess()

    async def _send_first_guess(self):
        await asyncio.sleep(Config.GUESS_DELAY)
        await self.client.send_message(self.chat_id, Config.START_WORD)
        self.last_guess = Config.START_WORD
        self.used_words.add(Config.START_WORD)
    ...
    def _update_constraints(self, word: str, hint: str):
        ...
        self.greens.update(local_greens)
    ...
    def _best_guess(self) -> str:
        filtered = [
            ...
            if self._valid(w) and w not in self.used_words
        ]
